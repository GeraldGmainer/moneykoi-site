    ...
    <div class="mx-auto flex h-16 sm:h-20 lg:h-32 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div class="hidden sm:flex items-center gap-8 text-base font-medium text-slate-600">
        <nav class="flex items-center gap-6 lg:gap-8">
          <NuxtLink :to="localePath('/features')" class="transition hover:text-slate-900"
            active-class="text-[#FF5F2E] font-semibold">
            {{ $t('header.features') }}
          </NuxtLink>

          <NuxtLink :to="localePath('/roadmap')" class="transition hover:text-slate-900"
            active-class="text-[#FF5F2E] font-semibold">
            {{ $t('header.roadmap') }}
          </NuxtLink>

          <NuxtLink :to="localePath('/faq')" class="transition hover:text-slate-900"
            active-class="text-[#FF5F2E] font-semibold">
            {{ $t('header.faq') }}
          </NuxtLink>

          <UButton to="https://my.moneykoi.app" :label="$t('header.goToApp')" variant="solid" external
            class="bg-[#FF5F2E] hover:bg-[#e04f25] text-white font-bold px-6 py-2.5 rounded-full transition-all shadow-md shadow-orange-200 hover:shadow-lg active:scale-95" />
        </nav>
      </div>

      <button :aria-expanded="open ? 'true' : 'false'" :aria-label="$t('header.menuAriaLabel')"
          ...
          <nav class="mx-auto max-w-6xl px-4 py-4 text-base font-medium text-slate-700">
            <ul class="space-y-2">
              ...
              <li class="mt-4 pt-4 border-t border-orange-100 flex items-center gap-4 px-3">
                <button :class="locale === 'de' ? 'text-slate-900 font-bold' : 'text-slate-500'"
                  @click="setLocale('de'); open = false">
                  Deutsch
                </button>
                <span class="text-slate-300">•</span>
                <button :class="locale === 'en' ? 'text-slate-900 font-bold' : 'text-slate-500'"
